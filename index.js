const quoteApiUrl = "https://api.quotable.io/random?minLength=80&maxLength=150";
...
userInput.addEventListener('input', () => {
    ...
    // Create an Array from recieved quoteChars
    ...
    // Loop the each characters in quote
    quoteChars.forEach((char, index) => {
        if (char.innerText == userInputChars[index]) {
            char.classList.add('success');
        }
        // If user hasn't enterned anything or backspace
...
function updateTimer() {
    if (time == 0) {
        // End test if timer reaches to 0
        displayResult();
    }
    else {
